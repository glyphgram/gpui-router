use gpui::{
    App, AppContext, Context, IntoElement, ParentElement, Render, RenderOnce, Styled, Window, WindowOptions, div, rgb,
    white,
};
use gpui_router::{NavLink, Route, Routes};
...
impl RenderOnce for SettingsRouter {
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        div()
            ...
            .child(div().child(
                "This nested router only handles the /settings subtree via Routes::new().basename(\"/settings\").",
            ))
            .child(settings_nav())
            .child(
                Routes::new()
